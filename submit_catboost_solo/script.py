...
class DeepFM(nn.Module):
    """code/fm_model.py::DeepFM 와 동일 (수동 동기화 유지). 3번째 스태킹 모델 —
    CatBoost(그리디 트리)·MLP(quantile-embedding dense 결합)와 달리 명시적 order-2
    인수분해 상호작용 항을 쓴다. pitcher_id/batter_id를 저랭크(k) 임베딩 필드로 포함한다."""

    def __init__(self, cat_dims, num_numeric, bin_edges, k=8, deep_hidden=(128, 64), dropout=0.3):
        super().__init__()
        self.k = k
    ...
    def forward(self, x_cat, x_num):
        cat_k = [emb(x_cat[:, i].long()) for i, emb in enumerate(self.cat_embed_k)]
        fields_k = list(cat_k)
        if self.num_embed_k is not None:
            num_k = self.num_embed_k(x_num)
            num_k = num_k.view(x_num.shape[0], self.n_num, self.k)
            fields_k += [num_k[:, i, :] for i in range(self.n_num)]
        V = torch.stack(fields_k, dim=1)

        sum_sq = V.sum(dim=1) ** 2
        sq_sum = (V ** 2).sum(dim=1)
        fm_order2 = 0.5 * (sum_sq - sq_sum).sum(dim=1, keepdim=True)
...
# 투수x타자 페어 상대전적 5개 (code/train.py::apply_pair_matchup_static 와 동일, 수동
# 동기화 유지). test.csv(season=2025)는 페어 자체의 역사가 없고, 행별 독립 추론 규칙상
# test.csv 행끼리 서로 다른 갱신값을 가지면 안 되므로, dopip.py Full Retrain 단계에서
# train.csv 전체(2019~2024)로 미리 계산해 model/pair_matchup_lookup.csv로 동봉한 정적
# "최종" 누적치를 그대로 병합만 한다(season_end_lookup.csv/pitchmix_lookup.csv와 동일 관례).
PAIR_ID_COLS = ["pitcher_id", "batter_id"]
PAIR_COLS = ["pair_n", "pair_success_count", "pair_success_rate",
             "pair_rate_gap_pitcher", "pair_rate_gap_batter"]


def add_pair_matchup(df, model_dir):
    lookup = pd.read_csv(os.path.join(model_dir, "pair_matchup_lookup.csv"))
    df = df.copy()
    merged = df[PAIR_ID_COLS].merge(lookup, on=PAIR_ID_COLS, how="left")
    pair_n = merged["pair_n"].fillna(0).values
    pair_success = merged["pair_success_count"].fillna(0).values
    with np.errstate(invalid="ignore", divide="ignore"):
        pair_rate = np.where(pair_n > 0, pair_success / pair_n, np.nan)
    df["pair_n"] = pair_n
    df["pair_success_count"] = pair_success
...
def main():
    ...
    # 2d. 투수x타자 페어 상대전적 5개 — 2026-08-22 세션에 dual-regime 로컬 검증(CatBoost/
    # MLP 단독 개선 포함)을 통과해 채택했으나 실전 리더보드 1028.75(직전 확정 최고 1041.40
    # 대비 -12.65)로 기각 확정. 번들이 이 5개를 cat_feature_cols/mlp num_cols 어디에도
    # 요구하지 않으므로 호출을 비활성화한다(add_pair_matchup 함수 자체는 남겨둠).
    # df_test = add_pair_matchup(df_test, MODEL_DIR)
    ...
    # [임시: CatBoost 단독 실전 테스트, 2026-08-23] MLP/블렌드를 완전히 건너뛰고
    # CatBoost 확률만 그대로 제출한다 — "우리 현재 피처 레시피(F1필터+cutoff7+
    # 시즌진행분+TE-residual+coarse pitchmix)의 CatBoost 단독 실전 점수가 팀원이 보고한
    # ~1,000점대에 도달하는지" 확인용. 프로덕션 submit.zip(블렌드)은 별도로 그대로 둔다.
