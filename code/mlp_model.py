...
# (7-seed 앙상블 실측 BSS 점수 780.92, 단일 모델 690.62).
#
# 2026-08-23 재검증(채택 시도했으나 실전 결과로 되돌림): dual-regime 스크리닝(cutoff7
# blend +3.88, season==2023 blend +3.99)이 이례적으로 일치해 15-seed로 채택했으나,
# dopip.py의 자체 test.py 비교가 KEEP_REF로 나와 스크리닝과 모순됐다(사용자가 그래도
# 실전 제출을 진행하기로 결정). 실전 리더보드 결과: 1034.94 — 직전 확정 7-seed
# 1041.40 대비 -6.46. 이 델타는 이 프로젝트의 CatBoost thread_count 노이즈 밴드
# (7~19pt)보다 작아 확정적 증거는 아니지만, 로컬 KEEP_REF + 실전 마이너스 두 신호가
# 겹쳐 스크리닝의 +4pt를 노이즈로 보는 쪽에 무게가 실린다. 사용자 판단으로 7-seed로
# 롤백. `code/experiment_seed_count_1521.py`는 남겨두되(재현 기록용), 15-seed는
# 프로덕션 기준으로 다시 쓰지 않는다.
#
# 2026-08-24: 사용자 판단으로 20-seed 재도전. cutoff7 로컬 스크리닝에서 우리
# CatBoost 5seed 배깅 + MLP 20seed 블렌드가 실제 레퍼런스(753.37) 대비 +7.98로
# 나왔고(`code/experiment_3way_stack.py`, 팀원 CatBoost는 확실히 기각되어 제외),
# 15-seed의 실전 -6.46 전례가 있어 확신할 수는 없지만 사용자가 실전 제출로
# 검증해보기로 결정. 7-seed(기존) + `code/experiment_seed_count_1521.py`의
# NEW_SEEDS(8개) + `code/experiment_seed_count_20.py`의 NEW_SEEDS_5(5개) = 20개.
ENSEMBLE_SEEDS = [42, 123, 7, 2024, 99, 555, 31337, 1, 2, 3, 4, 5, 6, 8, 9, 10, 11, 12, 13, 14]
