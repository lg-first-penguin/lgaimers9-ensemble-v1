...
A private team pipeline for the LG Aimers / DACON competition (야구 투구 제어 성공 예측, `control_success` binary target, evaluated via Brier Skill Score). Built for Ubuntu 24.04 (WSL2), Python 3.11. Was CatBoost-based; migrated to a PyTorch **Tabular MLP** (embeddings for low-cardinality categoricals + a small MLP over embeddings/numeric features), originally motivated by a Colab prototype that reported ~1500 — that number turned out to be invalid (a random 80/20 split fallback leaking player identity across train/val, not the intended season==2024 holdout; see TABULAR_MLP_REPORT.md §2). On the real, leak-free season==2024 holdout, tuned CatBoost (818.54) actually beats the MLP ensemble alone (789.58) — but on the real competition leaderboard the MLP ensemble scored *higher* than CatBoost, the opposite ranking (see TABULAR_MLP_REPORT.md §4 for why local validation and the real leaderboard disagree here). The current production model is **not MLP alone** — it's a **CatBoost + MLP ensemble blend**, which beat both individual models on the local holdout and scored **924 on the real competition dashboard** after submission (under the alpha-weighted-average version described below) — the local holdout has consistently underestimated real performance throughout this project (see TABULAR_MLP_REPORT.md §4). The combination step was originally a fixed weighted average (`alpha≈0.59` toward CatBoost, 851.19 local) but has since been replaced with a **stacking meta-model** (2-feature logistic regression over `[catboost_pred, mlp_pred]`, no fixed alpha) after rolling-origin cross-validation showed it consistently beats the alpha blend (EXPERIMENTS.md §9); after adopting quantile (PLE) numerical-feature embedding in the MLP component (n_bins=24, EXPERIMENTS.md §15.3-§15.4), local score reached **879.34**, resubmitted and scored **957.80417 on the real competition dashboard** (up from the prior alpha-blend-era 924; EXPERIMENTS.md §15.5), the local holdout underestimating real performance yet again, consistent with every prior submission in this project. That 957.80417 submission turned out to have a dormant bug: the trackman merge's `season`-inclusive match key meant all 64 trackman-derived columns were structurally guaranteed to be constant 0 at real inference (test.csv is always season 2025, `trackman_history.csv` only covers 2019–2024), invisible to local validation because the season==2024 holdout happens to fall within trackman's coverage (EXPERIMENTS.md §16). Chasing a fix (season-dropped fallback matching, then an asof-cumulative season-cutoff redesign) led to a broader finding: trackman carries no reliable signal under proper multi-season validation (2023+2024 average delta ≈ 0 for every matching strategy tried, EXPERIMENTS.md §17) — corroborated independently by a teammate's separate investigation of 3 different trackman approaches, all negative (EXPERIMENTS.md §18). **Trackman is now dropped from the pipeline entirely.** That same teammate investigation also surfaced a bigger problem with this project's validation convention itself: single-season (season==2024) holdout scores are dominated by that season's league-wide baseline volatility, not model quality — a `game_type` F(퓨처스)/R(1군) success-rate relationship inverted in 2023 (F was up to +20.5%p above R through 2022, then −3.0%p below from 2023 on) and `game_type` is CatBoost's #1 feature importance, so a season==2023 holdout of the *same* model collapses to near-zero even though season==2024 looked fine (EXPERIMENTS.md §18.1-§18.2, reproduced independently on this repo's own data). The fix — dropping `game_type=='F'` rows from seasons ≤2022 in training only (`code/train.py::apply_f1_filter`), validation/inference data untouched — is now adopted (EXPERIMENTS.md §18.3): +229.18 avg over a 2023+2024 holdout (2024 alone actually looks like a small loss, −19.56; the 2023 recovery, +477.91, is what makes it worth adopting — 2024-only would have hidden this). With both changes (trackman dropped, F1 filter adopted) the pipeline was re-run end-to-end; local season==2024 score reached **777.61** (EXPERIMENTS.md §19) — not directly comparable to the pre-change 879.34/957.80417 since the validation convention itself is now understood to be unreliable in isolation (see "Train/eval split convention" below). A proper `train<2023`/`val∈{2023,2024}` split was considered to make that validation convention itself more trustworthy but rejected — it would force `apply_f1_filter` to remove *all* F rows from training (since the only training seasons left, 2019–2022, are exactly what the filter drops), degenerating to the "F2, full removal" variant already shown to underperform F1 (EXPERIMENTS.md §18.2, §20); the 2023+2024 check instead stays a manual audit tool (`--holdout` flag on the experiment scripts) for judging individual candidate changes, not the routine promotion decision. Since F1 filter changed the training distribution CatBoost's hyperparameters were originally tuned on, `code/tune.py` (Optuna, 40 trials) was re-run on the current F1-filtered/trackman-free feature set and found new params **+32.31 pts better standalone** (721.66 vs 689.35) — adopted into `code/catboost_model.py::CATBOOST_PARAMS`, and after a full pipeline re-run the blend reached **794.77** locally. Resubmitted under this architecture (trackman dropped + F1 filter + re-tuned CatBoost) and scored **971 on the real competition dashboard** (EXPERIMENTS.md §22) — the project's highest real score yet, up from the prior 957.80417, despite the local number (794.77) looking lower than that submission's local number (879.34) — consistent with §18.1's finding that the season==2024-only local metric doesn't reliably reflect the true effect of changes like these. A revisited TabM prototype (BatchEnsemble-style efficient ensemble) with corrected initialization confirmed its original failure hypothesis but still didn't beat the Deep Ensemble MLP, so it stayed unadopted (EXPERIMENTS.md §11, TABULAR_MLP_REPORT.md §5.1). See TABULAR_MLP_REPORT.md for the full trail and EXPERIMENTS.md for the raw experiment log.
...
Data must be manually downloaded into `open/data/` (`train.csv`, `test.csv`, `trackman_history.csv`, `sample_submission.csv`) — this directory is gitignored. `trackman_history.csv` is part of the official dataset but is no longer read by any script in the main pipeline (`code/train.py`, `code/test.py`, `dopip.py`, `submit/script.py`) — trackman was tried and dropped, see "Trackman feature engineering — removed" below.
...
The model actually shipped in `submit/` is **not** the MLP ensemble alone — it's a stacked combination of two independently-trained models: CatBoost (hyperparameters hardcoded in `code/catboost_model.py::CATBOOST_PARAMS`, from the team's own Optuna search via `code/tune.py` — most recently re-run on the current trackman-free/F1-filtered feature set, EXPERIMENTS.md §21) and the Tabular MLP ensemble described below. Final prediction = `sigmoid(w_cat * catboost_pred + w_mlp * mlp_ensemble_pred + intercept)`, where `w_cat`/`w_mlp`/`intercept` are fit per-run by a 2-feature `LogisticRegression` (`code/blend_model.py::fit_meta_model`) trained on `[catboost_pred, mlp_pred] -> y` over the validation split — this replaced an earlier fixed weighted-average blend (`alpha * catboost_pred + (1-alpha) * mlp_pred`, `alpha≈0.59`, chosen by a 0.01-step grid search) after rolling-origin cross-validation showed the stacking meta-model consistently beats the alpha blend (avg +14.22 pts across 3 time-based folds, 3/3 folds won — see EXPERIMENTS.md §9). Trackman-derived features and the 2022-and-earlier `game_type=='F'` training rows were both removed/filtered as of EXPERIMENTS.md §17-§19 (see "What this is" above and "Train/eval split convention" below for why), and CatBoost was subsequently re-tuned on that changed data (EXPERIMENTS.md §21) — local score (season==2024 holdout) is **794.77**, not directly comparable to the pre-change 879.34/957.80417 numbers since both the feature set and the validation convention's reliability have changed. This architecture scored **971 on the real competition dashboard** (EXPERIMENTS.md §22), the project's highest real score yet despite the lower local number — beating CatBoost alone and the MLP ensemble alone individually is still true at this stage but the absolute numbers from EXPERIMENTS.md §15 predate this change. See TABULAR_MLP_REPORT.md §6 for the original blending story (why it helped despite the two models' predictions being highly correlated, 0.9991) and EXPERIMENTS.md §9 for the stacking upgrade.
...
- `CAT_COLS = ['top_bottom', 'game_type', 'base_state', 'pitcher_hand', 'batter_hand', 'pitcher_team_id', 'batter_team_id']` (defined in `code/mlp_model.py`) are embedded, sized per-column via `embed_dim_for_cardinality` (fastai-style heuristic, `min(50, (cardinality+1)//2)`, floor 4). Every other feature column is numeric (median-imputed, standardized, then quantile-embedded as described above). Two categorical columns were tried and explicitly reverted after real-data testing — see the `CAT_COLS` comment in `code/mlp_model.py` and EXPERIMENTS.md for the full story:
  ...
  - `season`: validation is *entirely* season==2024, which never appears in training (train is season<2024). Embedding it means every validation row shares one untrained/random embedding vector — a systematic miscalibration (660 → 351). As a numeric feature the model can at least extrapolate smoothly to an unseen season value, which matters because the real eval `test.csv` is season 2025 (also unseen in `train.csv`'s 2019–2024) — the same failure mode would hit production, not just the validation experiment. Any feature guaranteed to take on new values at inference time should stay numeric, not become an embedded category.
...
`submit/script.py` is the standalone inference entrypoint used by the competition server — it does **not** import from `code/`. It duplicates the `TabularMLP` class definition, `add_engineered_features`, `apply_preprocessing`, and the blend inference logic (CatBoost `predict_proba` + MLP ensemble-averaging loop over `bundle["mlp_bundle"]["members"]`, combined via the sigmoid of `bundle["meta_model"]`'s `w_cat`/`w_mlp`/`intercept`) inline. It does *not* need to duplicate any CatBoost training/class code — `CatBoostClassifier` unpickles directly as a fully-trained object, `submit/script.py` just calls `.predict_proba()` on it. Keep the duplicated pieces in sync manually if `code/mlp_model.py`, `code/catboost_model.py`, `code/blend_model.py`, or `code/train.py` change — this is the same manual-sync convention the CatBoost-era pipeline used for `add_engineered_features`. It no longer loads `trackman_history.csv` at all (see below).

**Trackman dropped entirely (EXPERIMENTS.md §16-§17, §19)**: an earlier version of this pipeline joined `trackman_history.csv` onto the main frame via a `match_cols` fingerprint that included `season`; since the real eval `test.csv` is always season 2025 and `trackman_history.csv` only covers 2019–2024, that `season` equality condition structurally never matched at real inference — all 64 trackman-derived columns were silently constant 0 for every row of the 957.80417 submission, invisible to local validation because the season==2024 holdout happens to fall within trackman's coverage (EXPERIMENTS.md §16). A season-dropped inference-only fallback match recovered some of that loss locally (+22.17), and a further redesign (asof-cumulative season-cutoff matching, applied consistently at train *and* inference time so there's no train/serve distribution skew) recovered more (+8.91 over dropping trackman outright — but only under the single-season 2024 holdout that turned out to be unreliable, see below). Once validated under a proper 2023+2024 multi-season holdout, that advantage disappeared: 2023+2024 average delta between the best trackman-matching scheme and no trackman at all was ≈0 (−1.35), with individual seasons swinging ±26-29 pts in opposite directions — i.e. no reliable signal, just noise (EXPERIMENTS.md §17.3). A teammate's independent investigation (3 different trackman approaches: situational-fingerprint join, pitcher-ID career physical stats, real-time in-game state) reached the same conclusion (all negative, EXPERIMENTS.md §18). **Trackman is now fully removed** — `code/train.py::process_trackman_features_safe` no longer exists, `dopip.py`'s full-retrain step no longer merges it, and `submit/script.py` no longer loads `trackman_history.csv` or does any trackman merge. `open/data/trackman_history.csv` remains available if a future, genuinely different angle is worth trying.

**F1 filter (`code/train.py::apply_f1_filter`, EXPERIMENTS.md §18)**: `train = train[~((train['game_type'] == 'F') & (train['season'] <= 2022))]`, applied to training data only (in `code/train.py::main()` and `dopip.py`'s full-retrain step) — never to `code/test.py`'s or `submit/script.py`'s validation/inference data. Motivated by a teammate-reported and independently-reproduced finding: `game_type`'s F(퓨처스/2군)/R(1군) success-rate relationship inverted starting in 2023 (F was up to +20.5%p above R through 2022, then flipped to −3.0%p below R from 2023 on), and since `game_type` is CatBoost's #1 feature importance, training on the pre-2023 (now-contradicted) relationship actively hurts — most visibly on a season==2023 holdout, where the unfiltered model's score collapses near zero even though season==2024 alone looks fine. 2023+2024 average score improved +229.18 after filtering (2024 alone actually shows a small loss, −19.56 — the win is entirely from 2023's recovery, +477.91, which a 2024-only validation convention would never surface).
...
Current convention: **season == 2024 is held out entirely as validation; seasons < 2024 are training-only** (`train_df['season'] < 2024` / `== 2024`). This replaced an earlier chronological 80/20 positional split (`sort_values(['season','game_month','game_dayofweek'])` + `iloc[:split_idx]`) that, in practice, put 100% of 2024 plus part of Sept–Oct 2023 into validation — i.e. it was already close to "2024-only holdout" but not exact and not intentional-looking. If touching the split again, verify actual row-count/season composition with a quick pandas check rather than trusting the code's comments — the "time filter" logic elsewhere in this codebase has a documented gap (see below).

**Known reliability gap (EXPERIMENTS.md §18.1-§18.2)**: this single-season convention has since been shown to be sensitive to that season's league-wide baseline volatility, not just model quality — `game_type`'s F/R success-rate relationship inverted starting in 2023 (see `apply_f1_filter` above), so the *same* model can look fine on a season==2024 holdout and collapse near-zero on season==2023, purely because of which season happens to be held out, not a change in the model. Every experiment result in this codebase computed on the season==2024 holdout alone (essentially everything before EXPERIMENTS.md §18) should be read with this caveat — it wasn't necessarily wrong, but it wasn't cross-checked against a second season either. `code/experiment_f1_filter.py --holdout {2023,2024}` and `code/experiment_trackman_asof9key.py --holdout {2023,2024}` support re-running any given change against both seasons.

**Deliberately not baked into production** (`code/train.py`/`code/test.py`/`dopip.py` still use season==2024 alone for reference-model comparison and full-retrain epoch/iteration selection) — this was a real option that was considered and rejected. Making it a true dual-season holdout would require excluding season 2023 from training (`train < 2023` instead of `train < 2024`, validating on `{2023, 2024}` both), which directly conflicts with `apply_f1_filter`'s intent: F1 only removes the *stale* pre-2023 F rows specifically so the *correct*, post-2023 F/R relationship stays available to train on. Pull season 2023 out of training and F1 filtering the remaining 2019–2022 data removes *all* F rows from training (nothing left with the corrected relationship) — equivalent to the "F2, full removal" variant a teammate already showed underperforms F1 (EXPERIMENTS.md §18.2). So the 2023+2024 check stays a manual audit step for evaluating individual candidate changes (as done for trackman in EXPERIMENTS.md §17.3 and F1 itself in §18.3), not the routine per-run promotion decision — the production split keeps season 2023 in training to preserve F1's benefit, at the cost of `code/test.py`'s promotion decision still resting on a single (2024) season.
...
**Not migrated to Tabular MLP**: `code/train_x30.py`, `code/test_x30.py`, `code/train_rd30.py`, `code/test_rd30.py`, `code/tune.py` (Optuna search) and `code/train.last.py` are all still CatBoost-based and were left untouched by the MLP migration — they're experimental/historical, not part of the `dopip.py` main path. If reviving one of these split strategies, it needs the same CatBoost→`code/mlp_model.py` rewrite `train.py`/`test.py`/`dopip.py` got.

### Trackman feature engineering — removed (EXPERIMENTS.md §16-§17, §19)

`code/train.py` used to have a `process_trackman_features_safe` function that joined `trackman_history.csv` onto the main frame by aggregating (mean/std) pitch metrics grouped by a `match_cols` fingerprint (season, game_month, game_dayofweek, inning, top_bottom, balls/strikes/outs_before, pitcher/batter_hand) crossed with `pitch_type_group`. It's gone now — see "Trackman dropped entirely" above for why (no reliable signal under proper multi-season validation, confirmed both internally and by an independent teammate investigation). `code/train_x30.py`/`code/train_rd30.py` (the untouched CatBoost-era split-strategy variants below) still reference a lighter trackman join of their own (`build_pitcher_consistency_features`) — that's a separate, never-migrated code path and wasn't touched by this removal.
