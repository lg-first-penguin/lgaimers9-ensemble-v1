...
`submit/script.py` is the standalone inference entrypoint used by the competition server — it does **not** import from `code/`. It duplicates the `TabularMLP` class definition, `add_engineered_features`, the trackman feature merge, `apply_preprocessing`, and the blend inference logic (CatBoost `predict_proba` + MLP ensemble-averaging loop over `bundle["mlp_bundle"]["members"]`, combined via the sigmoid of `bundle["meta_model"]`'s `w_cat`/`w_mlp`/`intercept`) inline. It does *not* need to duplicate any CatBoost training/class code — `CatBoostClassifier` unpickles directly as a fully-trained object, `submit/script.py` just calls `.predict_proba()` on it. Keep the duplicated pieces in sync manually if `code/mlp_model.py`, `code/catboost_model.py`, `code/blend_model.py`, or `code/train.py` change — this is the same manual-sync convention the CatBoost-era pipeline used for `add_engineered_features`.

**Trackman merge `season` fallback (`submit/script.py::merge_trackman_features`, EXPERIMENTS.md §16, inference-only)**: the trackman merge's `match_cols` includes `season`, and the real eval `test.csv` is always season 2025 while `trackman_history.csv` only covers 2019–2024 — so the `season` equality condition structurally never matches at real inference, meaning all 64 trackman-derived columns were silently constant 0 for every row of the 957.80417 submission (confirmed empirically, EXPERIMENTS.md §16). Local validation didn't catch this because its holdout season (2024) *is* within trackman's coverage, so local matching actually succeeds — a blind spot specific to the local/real season split, not something `code/train.py`'s local val can surface. `merge_trackman_features` now retries with `season` dropped from the match key whenever the primary (season-inclusive) match fails entirely for a row — safe *only* for inference, because the eval season is always chronologically after every season in `trackman_history.csv`, so no future information can leak in. This fallback is deliberately **not** applied in `code/train.py::process_trackman_features_safe` (training/validation feature construction) — there, `season` equality is currently the only thing preventing an earlier-season training row from matching a later-season trackman aggregate, so dropping it would introduce a real leak. Locally simulated on the season==2024 holdout with season==2024 removed from `trackman_history.csv` (to reproduce the real blind-season condition): CatBoost+MLP blend recovered from 730.25 (season-strict, fully blind — matches real deployment) to 752.42 (fallback) — a real but partial recovery (theoretical ceiling if trackman could see the eval season directly is 880.38). No retraining needed for this fix — only `submit/script.py`'s inference-time merge changed, so `submit/model/final_retained_model.pkl` is unchanged and only `submit.zip` needs rebuilding.
