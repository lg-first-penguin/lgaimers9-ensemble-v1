...
**Current production model (as of 2026-08-28, real leaderboard 1117.03 — this repo's confirmed best):** "candidate B raw" (yudam-recipe port) **with the 64 trackman situational columns removed from both models**. CatBoost (yudam v2 HP, 5-seed bagging, 79 features) + Tabular MLP ensemble (7-seed, **raw StandardScaler concat — NO quantile/PLE**, `bin_edges=None`, 68 numeric features) combined via a 2-input logistic stacking meta-model (w_cat=2.0117/w_mlp=1.9848/intercept=-2.0284, reused from candidate B raw). Feature set: coarse pitchmix + season-progression (success_rate + reverse_rate) + Track A TE-residual (→CatBoost only) + same_hand (→MLP only) + F1 filter — **the trackman situational 10-key physical-metric join (64 derived cols) is gone**: it collapses to dead per-column constants at 2025 inference (season ∈ match_cols) and that dead constant miscalibrates CatBoost, so removing it gained +24.03 real (see the 2026-08-28 trackman64-removal paragraph below). **PLE was final-rejected 2026-08-28** (B-with-PLE real 1077.25 vs candidate B raw 1092.998 = clean −15.75). `submit/model/final_retained_model.pkl` + `submit.zip` hold the trackman64-removed model (hand-built via `code/build_trackman64_removed_bundle.py`); **`code/train.py` + `code/test.py` + `dopip.py` now carry the `is_trackman64` filter too, so `python dopip.py` reproduces the 1117 recipe** (see the dated update below). `open/reference/best_model.pkl` is still candidate B raw and auto-supersedes on the next `code/test.py` run. `submit/script.py` unchanged (bundle's feature lists drive selection). candidate B raw backed up at `open/former_model/submit_pre_trackman64removed_20260828_185224/`. The meta weights in the 1117 bundle were reused from candidate B raw (fit with trackman64 alive) — a proper stage-1 re-fit on the trackman64-free feature set may gain a little more.
