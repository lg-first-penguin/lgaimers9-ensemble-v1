...
The model actually shipped in `submit/` is **not** the MLP ensemble alone — it's a weighted-average blend of two independently-trained models: the tuned CatBoost (see "제출 모델 만들기 설명서" era tuning — hyperparameters extracted via `.get_params()` from the old tuned reference, hardcoded in `code/catboost_model.py::CATBOOST_PARAMS`, all derived from the team's own Optuna search on official data) and the Tabular MLP ensemble described below. Final prediction = `alpha * catboost_pred + (1 - alpha) * mlp_ensemble_pred`, with `alpha` (currently ≈0.59, CatBoost's weight) chosen per-run by a 0.01-step grid search (`code/blend_model.py::sweep_alpha`) against the validation split. This blend scored 851.19 locally, beating both CatBoost alone (818.54) and the MLP ensemble alone (789.58) — see TABULAR_MLP_REPORT.md §6 for the full story and why blending helped despite the two models' predictions being highly correlated (0.9991).

- **Bundle schema**: `{"catboost_model": <CatBoostClassifier>, "mlp_bundle": {...the MLP ensemble bundle below...}, "alpha": float, "catboost_best_iteration": int}`. `catboost_model` is a plain library object (not project-defined), so it unpickles fine anywhere `catboost` is installed — no `code/` package dependency, same reasoning as the MLP bundle being a plain dict.
...
- CatBoost's categorical features are just `['game_type', 'base_state']` — `top_bottom` is already mapped to int 0/1 by `process_trackman_features_safe` before CatBoost ever sees it, so CatBoost doesn't need it declared as categorical (confirmed via `model.get_cat_feature_indices()` on the old tuned reference).
...
The model is a small PyTorch net (`TabularMLP`): an `nn.Embedding` per low-cardinality categorical column, concatenated with the (imputed + standardized) numeric columns, fed through `Linear(128) → BatchNorm → ReLU → Dropout(0.3) → Linear(64) → BatchNorm → ReLU → Linear(1) → Sigmoid`. Trained with `BCELoss` + `AdamW` (lr 0.003, weight_decay 0.01), batch size 4096.

- `CAT_COLS = ['top_bottom', 'game_type', 'base_state', 'pitcher_hand', 'batter_hand', 'pitcher_team_id', 'batter_team_id']` (defined in `code/mlp_model.py`) are embedded, sized per-column via `embed_dim_for_cardinality` (fastai-style heuristic, `min(50, (cardinality+1)//2)`, floor 4). Every other feature column is numeric (median-imputed, then standardized). Two categorical columns were tried and explicitly reverted after real-data testing — see the `CAT_COLS` comment in `code/mlp_model.py` and EXPERIMENTS.md for the full story:
...
4. **Full retrain**: regardless of step 3's outcome, reloads whichever bundle is now `open/reference/best_model.pkl`. If it's a compatible blend bundle with an MLP ensemble of the same size as `ENSEMBLE_SEEDS`, it reuses: each MLP seed's own `best_epoch` (+ 5-epoch buffer) as that seed's full-retrain epoch budget, CatBoost's `catboost_best_iteration` (+ 50-iteration buffer via `CATBOOST_ITERATION_BUFFER`) as its full-retrain iteration count, and the reference's `alpha` as-is (not re-swept — there's no validation set at this stage). Both the MLP ensemble and CatBoost are retrained on the **entire** dataset (`is_train_split=False`, no held-out split, so no early stopping is possible), and the resulting blend bundle is written to `submit/model/final_retained_model.pkl`. I.e. the reference/eval model only decides *how many epochs/iterations to use, which preprocessing stats, and what alpha* — the actual submitted model is always retrained on all available data.

`submit/script.py` is the standalone inference entrypoint used by the competition server — it does **not** import from `code/`. It duplicates the `TabularMLP` class definition, `add_engineered_features`, the trackman feature merge, `apply_preprocessing`, and the blend inference logic (CatBoost `predict_proba` + MLP ensemble-averaging loop over `bundle["mlp_bundle"]["members"]`, combined via `bundle["alpha"]`) inline. It does *not* need to duplicate any CatBoost training/class code — `CatBoostClassifier` unpickles directly as a fully-trained object, `submit/script.py` just calls `.predict_proba()` on it. Keep the duplicated pieces in sync manually if `code/mlp_model.py`, `code/catboost_model.py`, `code/blend_model.py`, or `code/train.py` change — this is the same manual-sync convention the CatBoost-era pipeline used for `add_engineered_features`.
...
### Trackman feature engineering (`process_trackman_features_safe` in `code/train.py`)

Used by `train.py` and `dopip.py`'s full-retrain step (imported via `from code.train import process_trackman_features_safe`). Joins `trackman_history.csv` onto the main frame by aggregating (mean/std) pitch metrics grouped by a `match_cols` fingerprint (every column shared between main and trackman data — season, game_month, game_dayofweek, inning, top_bottom, balls/strikes/outs_before, pitcher/batter_hand) crossed with `pitch_type_group`, then pivoted so pitch types become columns.

`is_train_split=True` is intended to strip "future" trackman rows (relative to the max season/month seen in the input) before aggregating, to prevent leakage. In practice this filter is computed against whatever `df` is passed in at call time — in `train.py` it's called on the *full* un-split dataframe before the train/val split happens, and its cutoff (2024-10) happens to equal trackman_history.csv's own max date, so **the filter currently removes zero rows** and has no real effect. Because `match_cols` already includes `season`/`game_month`, cross-season leakage isn't actually possible via this merge regardless — but if the split strategy or data changes, don't assume this filter is doing anything without checking `df_trm_copy` row counts before/after.

`dopip.py`'s full-retrain step calls this with `is_train_split=False`, skipping the filter entirely by design (final submission model is meant to use all data).
...
- `open/reference/best_model.pkl` — current best-scoring bundle, source of truth for CatBoost's `catboost_best_iteration`, each MLP member's `best_epoch`, and `alpha`, all reused in full retrain. This file may still be a leftover raw `CatBoostClassifier` pickle, an MLP-only bundle, or an older single-model MLP bundle (missing `"catboost_model"`/`"mlp_bundle"` keys) until the next `dopip.py` run promotes a new blend bundle over it; `test.py` handles that transition gracefully (see pipeline flow above).
