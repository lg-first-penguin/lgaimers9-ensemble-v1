...
**Update (2026-08-31, still later): the two set-aside sweep candidates (`cat_teamhand`, `mlp_no_pid`) both dead — production stays 1126.77.** User asked to also try the candidates cat_team had beaten. `cat_teamhand` (team_id + hand both CatBoost categorical): blend Δ cutoff7 +10.97 / 2023 +3.26 / 2022 −5.20 / 2021 −34.73 ≈ cat_team on the realistic regimes, and `cat_hand` alone is a 2023 regime-flip (+5.95/−5.68) — no mechanism (hand cardinality 2–3, CatBoost's numeric split on a 2-value int == a categorical split), so not built. `mlp_no_pid` (drop pitcher_id/batter_id from MLP `num_cols` only — they were fed as raw standardized scalars of anonymized IDs): sweep `d_mlponly` blend Δ cutoff7 **+13.28** / 2023 **−6.76** / 2021 **−85.56** = textbook cutoff7-only-positive regime-flip, no monotone-with-data rescue, no 2025-collapse mechanism, and `cat_team + pidMLP` combo already tested at 2023 −4.85. **User chose to spend a real datapoint anyway.** Speed-built `code/build_catteam_nopidmlp_bundle.py` (reuse cat_team CatBoost 5-seed + meta **verbatim, not refit**; retrain only MLP 7-seed pid-removed on 1.37M, MLP num 68→66, CatBoost 79 unchanged), clean-room 5-row smoke 9.9s pass. **Real leaderboard 2026-08-31 23:39: 1102.76 — −24.02 vs cat_team 1126.77. REJECTED** — the local regime-flip called it correctly; same "cutoff7 small-positive = noise" as the M4 meta (−13.24), PLE (−15.75), G_interact catonly (−39.80). `submit/` restored to cat_team 1126.77 (pkl md5 `2df6f38f…`) from backup `open/former_model/submit_pre_nopidmlp_20260831_233611/` (which held the cat_team bundle, taken by `promote()` before the nopidmlp overwrite); `submit.zip` re-copied from `submit_catteam_20260831_223950.zip`. No `code/`/`dopip.py` change (the nopidmlp bundle was hand-assembled outside the main pipeline). Untracked: `code/build_catteam_nopidmlp_bundle.py`, `submit_catteam_nopidmlp_20260831_233609.zip`.

**Update (2026-09-01): "combine existing features via subtraction / division" (teammate tip) — Phase 1 wipeout, line closed, no production change.** A teammate reportedly gained a lot from ratio/difference recombinations of existing features but gave no specifics. Audit first: **subtraction is already pervasive and load-bearing here** — `{pitcher,batter}_season_rate_gap` (this-season rate − pre-season baseline, "biggest validated feature win in project history"), TE-residual `*_res` (obs TE − prior), `pitcher_recent{1,3,5}_gap`, `pitcher_relative_success`, `count_diff`, `pitcher_trend`, `matchup` are all A−B forms in production; `career_trend`/`career_acceleration`/month-YoY differences were all tried and rejected. **Division was the genuine gap** — only coarse pitchmix (count/total) and internal rate math, never explicit "rate A / rate B". So `code/experiment_yudam_ratio_features.py` (13 candidates, all official `asof_*` rate arithmetic, zero 2025-collapse risk; `a/(b+0.02)` additive smoothing) screened 6 groups at 1-seed cutoff7: **q_misscomp** (middle/ball, reverse/middle, strike/ball, middle/success), **q_form** (prevN/career ratios — the ratio analog of the adopted `_gap` differences), **q_matchup** (pitcher/batter success & middle ratios), **q_mix** (fastball/breaking), **q_all** (all 10 division feats), **d_novel** (reverse−middle, ball−strike, prev1_mid−middle new difference axes). Baseline blend 736.54. **All 6 groups blend-negative**: q_all −0.01 (MLP solo +10.20 fully cancelled by the meta), q_form −8.60 (CatBoost solo +8.36, flips), d_novel −9.34 (CatBoost solo +9.93), q_matchup −13.26 (MLP solo +18.82, blend collapses), q_mix −16.26, q_misscomp −19.84. Same structure as the G1–G4 Phase-1 wipeout (2026-08-29): a recombination helps one solo model but the 2-input logistic meta destabilizes and the blend drops. Confirms `scratchpad/FEATURE_THESIS.md`'s thesis (pure new arithmetic features have never worked here — only re-representation / removal / regime-alignment). **Follow-up (user: "could the stacking itself be the constraint w/ ratio features? 1-seed is thin"):** re-ran the CatBoost-solo movers (`q_form`, `d_novel`, `q_all`) as **catonly** and the MLP-solo movers (`q_matchup`, `q_mix`, `q_all`) as **mlponly**, all at **3-seed** cutoff7 (`--catonly`/`--mlponly` flags added to the script; `scratchpad/ratio_route_chain.sh`). Two findings: (1) **near-collinearity was real** — routing a ratio to ONE model stops the both-feed blend collapse (`q_form` −8.60→−0.47, `q_mix` −16.26→+1.27), i.e. the 2-input logistic meta *was* amplifying damage when both base models got correlated inputs; (2) **but there is no signal underneath** — the 1-seed "solo model jumped +18.82 / +9.93" were single-seed variance: at 3-seed `q_matchup` MLP solo goes +18.82→**−15.93**, `d_novel` CatBoost solo +9.93→**−6.42**, `q_all` MLP solo +10.20→**−30.65**. Best routed result is `q_mix` mlponly **+1.27** (below the +2 bar, the exact cutoff7-only-noise pattern that lost 5× on the real board). So changing the meta wouldn't rescue these — routing already removes the collinearity artifact and the features still add ≈0. The ceiling is informational, not architectural (and the meta layer itself was already swept — M1–M5, context-weights, calibration — with every real submission losing, e.g. M4 −13.24). **Line closed for good, no submission.** `submit/`, `code/train.py`, `dopip.py` untouched. Untracked: `code/experiment_yudam_ratio_features.py`, `scratchpad/ratio_route_chain.sh`, `scratchpad/ratio_{features_result,catonly,mlponly}.json`, `scratchpad/ratio_{phase1,route}.log`.

**Update (2026-09-01, later): three self-devised "structural" candidates (D1 career-rate CatBoost-drop / B monotone_constraints / D2 cold-start flags) all screen-rejected — production stays cat_team 1126.77, nothing built.** User asked for new plans grounded in "what has actually worked here" (trackman64 removal, cat_team, PLE removal — all either a train↔2025 structural-mismatch fix or a better representation of existing signal). Extended the experiment harness first (`code/experiment_yudam_common.py::run_experiment` gained `cb_cat_features` / `cb_params_extra` kwargs, threading `.astype(str)` team-id cast + params override — so experiments now run on the **real cat_team 1126.77 baseline**, not the 1117 one). New `code/experiment_structural_batch.py` (configs `d1_career_catdrop` / `b_monotone` / `d2_coldstart_flags`), 3-seed, cutoff7 + 2023 (deadline: only the two realistic regimes; teammate took Plan C = ABS-regime sample weighting in parallel). Results, all vs the cat_team baseline (cutoff7 blend 755.31 / 2023 785.70):
- **D1** (`asof_{pitcher,batter}_success_rate` removed from `cat_feature_cols` only, kept in MLP — rationale: career-cumulative rate is pre-ABS-contaminated for 2025 veterans): cutoff7 blend **−6.43**, **CatBoost solo Δ +0.04 / −0.07** (i.e. *exactly zero* — the career rate is fully redundant with the season-progression `_season_rate`/`_rate_gap` + `pitcher_recent{1,3,5}_gap` + TE-residual already in production, so dropping it from CatBoost changes nothing). The blend deltas (−6.43 cutoff7 / +3.28 2023) are pure MLP 3-seed noise (MLP solo swung −9.53 / +9.04 despite D1 not touching MLP features). Best-argued mechanism of the three, but the mechanism metric flatlined. Rejected.
- **B** (CatBoost `monotone_constraints`, 14 rate features: +1 on success-rate-family, −1 on middle/reverse-family — a capacity-*reducing* domain prior, explicitly not an Optuna HP search): cutoff7 blend **−8.93**, **CatBoost solo Δ −55.79** (2023 +18.20 — huge regime split). The monotone priors actively break structure CatBoost exploits on the ABS-regime holdout; plausibly the rate↔`control_success` relationship is non-monotone in the ABS era (very-high `asof_pitcher_success_rate` = pre-ABS-inflated veteran, so the tail bends the wrong way). Rejected hard.
- **D2** (`is_{pitcher,batter}_lowinfo` = `asof_*_n < 200` binary flags, fed to both models — let the model calibrate the "no player-specific signal" 2025 regime instead of imputing rookies to league mean): cutoff7 blend **−3.51** (CatBoost solo +1.15, MLP solo −2.37), 2023 blend −0.11 (CatBoost −7.82, MLP −9.69). Flags mildly hurt the MLP, matching the documented `intentional_walk` finding (9/9 flag variants net-negative, MLP −16~−52). Rejected.

**Common pattern: all three negative on cutoff7 (the decision fold), and the mechanism metrics confirm it's not measurement noise** (D1 = zero CatBoost movement / info redundant; B = −55.79 real CatBoost damage; D2 = MLP degradation). Re-confirms the `FEATURE_THESIS.md` / trackman64 lesson: **a feature-add / prune / regularization with no structural 2025-inference mechanism does not work here — a cutoff7 delta is not a reason to build.**

**Two more self-devised candidates then failed the same way (deadline session, 2026-09-01):**
- **`mlp_drop_season`** (remove `season` from MLP `num_cols` only, keep in CatBoost — hypothesis: `season=2025` is unseen at inference so the MLP's StandardScaler pushes every 2025 row to an out-of-range z, a systematic miscalibration): cutoff7 blend **−4.98**, MLP solo **−18.75** (2023 MLP solo −41.22). `season` is genuinely load-bearing signal for the MLP (ABS-regime / year effects), not a miscalibrating extrapolation — the hypothesis is **falsified**. The 2023 blend "+3.12" is a meta reweighting artifact (w_mlp collapsed to 0.109, w_cat inflated to 4.497 — the meta just zeroed the crippled MLP).
- **`cat_drop_season`** (remove from both models): cutoff7 blend −3.28 (CatBoost solo −10.70), 2023 blend −27.57. `season` is load-bearing for CatBoost too.

**Five self-devised structural candidates (D1/B/D2/mlp_drop_season/cat_drop_season), all negative at the cutoff7 screen.** BUT `mlp_drop_season` then **sign-flipped between environments on a re-run**: local (L4, numpy 1.x) cutoff7 blend −4.98 / MLP solo −18.75, vs Colab A100 (numpy 2.1.3) cutoff7 blend **+13.29** / MLP solo **+15.92** — the same config, same 3 seeds [1,2,3], ~35-pt MLP-solo swing purely from GPU-kernel + numpy-RNG nondeterminism. This is the documented few-seed MLP variance; neither number is trustworthy, and it's the exact "cutoff7 small-positive" band that lost 5× on the real board. Per user instruction ("submit *something*", capped downside since the scored submission is selectable), a **`mlp_drop_season` bundle was built anyway** (`code/build_mlpdropseason_bundle.py`: MLP 7-seed retrain with `season` out of `num_cols` (68→67), CatBoost 5-seed + meta + 3 lookups reused from cat_team verbatim, `season` stays in CatBoost's 79 `cat_feature_cols`; per-seed epochs = prod best_epoch + 5; clean-room 5-row smoke 9.1s pass). `submit_mlpdropseason_20260901_083054.zip` at repo root, staged `scratchpad/submit_mlpdropseason/`. **Submitted — real leaderboard 1104.96, −21.81 vs cat_team 1126.77. REJECTED.** `season` is genuinely load-bearing MLP signal; the "season=2025 MLP extrapolation miscalibration" hypothesis is falsified on the real board, and the Colab +13.29 was pure few-seed MLP variance (the local −4.98 had the right sign). This is the **6th** "cutoff7 small-positive / environment-disagreement → real-board loss" in this project (M4 −13.24 / PLE −15.75 / G_interact −39.80 / nopidmlp −24.02 / 15-seed −6.46 / mlp_drop_season −21.81). `submit/` was never promoted — production bundle md5 `2df6f38f…` = cat_team **1126.77** is unchanged and remains this repo's confirmed best; `submit.zip` (md5 `42a8bdc3…`) still holds it. `code/build_a10_bundle.py` A10 bundle (`submit_a10_20260901_065316.zip`, MLP flat-25 + CatBoost 7-seed) was also **submitted — real leaderboard 1119, −7.77 vs 1126.77. REJECTED** (inside the predicted near-zero noise band; epoch-policy + extra-seed changes don't help). **Net for this deadline session: 7 candidates (D1/B/D2/mlp_drop_season/cat_drop_season screened, mlp_drop_season + A10 real-submitted), all rejected. `submit/` was never promoted off cat_team 1126.77 (md5 `2df6f38f…`, `submit.zip` md5 `42a8bdc3…`) — it remains this repo's confirmed best and the final answer.**

**`A` (MLP full-retrain epoch flat-25 + CatBoost 5→7 seed) — built, NOT screen-tested (full-data-scale effect), zip staged for a possible real-board datapoint.** `code/build_a10_bundle.py` (2-stage: `--stage mlp` trains MLP 7-seed flat-25 in its own process → `mlp_bundle_ckpt.pkl`, `--stage cb` loads that + trains CatBoost 7-seed CPU-only + assembles — the split is because a single-process build got SIGKILLed right after MLP when the live CUDA context hit a WSL `dxgk` driver hiccup at CatBoost start). Reuses cat_team's meta + 3 lookup CSVs verbatim (md5-verified identical), cat_feature_cols 79 / num_cols 68 identical to production. Clean-room 5-row smoke passed (9.4s, probs 0.41–0.50). `submit_a10_20260901_065316.zip` at repo root + staged `scratchpad/submit_a10/` (`--promote-only` to adopt). Rationale is weak: candidate B raw hit real 1092.998 with per-seed epochs vs yudam's 1092.55 with flat-25, and the current production full-retrains at ~21 epochs avg vs flat-25, so this is near-certain noise — built only because the user can select which submission is scored (capped downside) and wanted a real datapoint.

New untracked scripts: `code/experiment_structural_batch.py` (configs d1_career_catdrop / d2_coldstart_flags / b_monotone / mlp_drop_season / cat_drop_season), `code/build_d1_bundle.py` (`--variant d1|b|d1b`, unused), `code/build_a10_bundle.py`, `scratchpad/colab_d1.py` + `colab_screen_cell.py` (single-cell Colab A100 screen runner), `scratchpad/{local_screen,local_d2,local_season,a10_2stage}.sh`; results `scratchpad/{local_screen_result,local_d2_result,local_season_result}.json`.
